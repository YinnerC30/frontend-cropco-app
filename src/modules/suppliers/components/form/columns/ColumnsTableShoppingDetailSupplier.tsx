...
export const columnsShoppingDetailSupplier: ColumnDef<any>[] = [
  {
    accessorKey: 'shopping.date',
    header: ({ column }: HeaderContext<any, unknown>) => {
      return <ButtonHeaderTable column={column} label={'Fecha:'} />;
    },
    cell: ({ row }) => {
      const dateShopping = row.original.shopping.date;
      return FormatDate({ date: dateShopping });
    },
  },
  {
