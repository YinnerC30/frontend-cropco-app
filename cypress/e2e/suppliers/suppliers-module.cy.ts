...
describe('Modulo de proveedores', () => {
  beforeEach(() => {
    cy.loginUser();
  ...
  it('Debe ingresar a la ruta correcta ', () => {
    ...
    cy.checkCurrentUrl('suppliers/view/all');
  });
  ...
  it('Debe contener los elementos necesarios', () => {
    ...
    // cy.get('button[data-testid="btn-export-all-suppliers"]')
    //   .should('exist')
    //   .should('be.visible');
  ...
  it('Ingresar al modulo usando el command', () => {
    ...
    cy.get('input[data-testid="input-command-search"]').type('proveedores');
    cy.get('div[data-testid="command-item-suppliers"]').click();
    cy.checkCurrentUrl('suppliers/view/all');
  });
...
describe('Encuentra registros de acuerdo a la cadena de busqueda', () => {
  const supplierData = {
    firstName: 'suppliertosearch',
    lastName: 'lasttosearch',
    email: '',
  };

  beforeEach(() => {
    supplierData.email = InformationGenerator.generateEmail();
    cy.loginUser();
    cy.createSupplier(supplierData);
  });

  it('Busqueda por nombre(s) del proveedor', () => {
    cy.typeOnInputBasicSearchBar(supplierData.firstName);
    cy.clickOnSubmitBasicSearchBar();
    cy.get('tbody tr')
      .filter(`:contains(${supplierData.firstName})`)
      .should('have.length.greaterThan', 0);
    cy.contains(supplierData.firstName);
    cy.contains(supplierData.lastName);
    cy.contains(supplierData.email);
  });
  it('Busqueda por apellido(s) del proveedor', () => {
    cy.typeOnInputBasicSearchBar(supplierData.lastName);
    cy.clickOnSubmitBasicSearchBar();
    cy.get('tbody tr')
      .filter(`:contains(${supplierData.lastName})`)
      .should('have.length.greaterThan', 0);
    cy.contains(supplierData.firstName);
    cy.contains(supplierData.lastName);
    cy.contains(supplierData.email);
  ...
  it('Busqueda por correo del proveedor', () => {
    cy.typeOnInputBasicSearchBar(supplierData.email.split('@')[0]);
    cy.clickOnSubmitBasicSearchBar();
    cy.get('tbody tr')
      .filter(`:contains(${supplierData.email})`)
...
describe('Creación de proveedores', () => {
  ...
  it('Debe crear un proveedor', () => {
    ...
    cy.getFormInput('company_name').type('Company name 1');
  ...
  it('Debe mostrar mensajes de error al intentar enviar el formulario vacio', () => {
    ...
    cy.contains('El nombre de la empresa debe tener minimo 4 caracteres');
...
describe('Modificación de proveedores', () => {
  ...
  it('Modificar proveedor existente', () => {
    cy.createSupplierAnd({}, ({ email, id }) => {
      cy.visit(`/app/home/suppliers/view/all?query=${email}`);
...
describe('Eliminación de proveedor', () => {
  ...
  it('Eliminar proveedor', () => {
    cy.createSupplierAnd({}, ({ email, id }) => {
      cy.openActionsMenuByField(
        email,
        `/app/home/suppliers/view/all?query=${email}`
      );
  ...
  // it('Intentar eliminar proveedor con ventas pendiente de pago', () => {});
});
...
describe('Eliminación de proveedores por lote', () => {
  before(() => {
    ...
    for (let index = 0; index < 2; index++) {
      cy.createSupplier(
        { firstName: 'SupplierToRemoveBulk' },
        { fastCreation: true }
      );
    }
    cy.logoutUser();
  ...
  it('Eliminar proveedores seleccionados', () => {
    cy.loginUser();
    cy.navigateToModuleWithSideBar('suppliers');
    cy.visit(`/app/home/suppliers/view/all?query=SupplierToRemoveBulk`);
    cy.wait(2000);
  ...
  // TODO: Implementar pruebas
  // it('Intentar eliminar proveedores con ventas pendiente de pago', () => {});

  // it('Intentar eliminar usuario con rol administrator en lote', () => {
  //   cy.loginUser();
  //   cy.navigateToModuleWithSideBar('suppliers');
  //   cy.visit(`/app/home/suppliers/view/all?query=Mantenimiento`);
  //   cy.wait(2000);
  //   cy.get('button[aria-label="Select all"]').click();
  //   cy.get('button[data-testid="btn-delete-bulk"]').click();
  //   cy.get('button[data-testid="btn-continue-delete"]').click();
  //   cy.contains(
  //     'No se pudieron eliminar los proveedores seleccionados, revisa que no tengan rol "Administrador"'
  //   );
  // });
});
...
describe('Ver registro de proveedor', () => {
  ...
  it('Ver registro de proveedor', () => {
    cy.createSupplierAnd({}, ({ email, id }) => {
      ...
      cy.getFormInput('first_name').should('have.value', 'SupplierName');
  ...
  it('Crear usuario con acceso unicamente al modulo de proveedores', () => {
    cy.createUser({ selectedModules: ['suppliers'] }).then((userData) => {
      cy.createSupplierAnd({}, (supplierData) => {
        ...
        cy.visit(`/app/home/suppliers/view/all?query=${supplierData.email}`);
        cy.wait(2000);
        ...
        // cy.get('button[data-testid="btn-export-all-suppliers"]').should(
        //   'be.enabled'
        // );
        ...
        cy.clickActionsButtonTableRow(supplierData.id);
  ...
  it('No tiene permisos para ver el listado de proveedores', () => {
    cy.createUserAnd({ selectedActions: ['create_supplier'] }, (userData) => {
      cy.createSupplierAnd({}, () => {
        ...
        cy.visit(`/app/home/suppliers/view/all`);
        cy.wait(2000);
